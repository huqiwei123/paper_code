...
class TestAddition(unittest.TestCase):
    ...
    def test_vehicle_communication(self):
        ...
        # 获取内容号为2的内容
        vehicle1.request_content = Enviroment.System.content_list[1]
    ...
    def test_cal_activityLevel(self):
        ...
        with vehicle_lock:
            vehicle_lock.wait()
            for vehicle in vehicle_list:
                other_vehicle_within_cluster = transfer_vehicle_no(vehicle.other_vehicle_within_cluster)
                other_vehicle_within_communication = transfer_vehicle_no(vehicle.vehicle_within_area)
                print(f"{vehicle.vehicle_no}号车辆的"
                      f"活跃度等级为{vehicle.activity_level}, "
                      f"通信范围内的其他车辆{other_vehicle_within_communication}"
                      f"簇内的其他车辆为{other_vehicle_within_cluster}, "
                      f"该车当前缓存状态为{vehicle.cache_status}, "
                      f"当前请求状态为{vehicle.request_status}, ", end=" ")
                if vehicle.request_content:
                    print(f"请求的内容为{vehicle.request_content.content_no}, " , end=" ")
                print(f"该请求经过了{vehicle.hop_count}跳, "
                      f"响应状态为{vehicle.cur_response_status},"
                      f"响应的车辆为{vehicle.request_vehicle} ")
            # 总请求量
            all_request = 0
            # 请求成功的数量
            success = 0
            ...
            total = sum(env.bs.cache_hit_ration)
            count = len(env.bs.cache_hit_ration)
            print(f"平均命中率为:{total / count}")
            plt.ylim(0, 1.0)
            # sampled_data = env.bs.cache_hit_ration[::10]
            # x = range(len(sampled_data))
            plt.plot(env.bs.cache_hit_ration)
            plt.title("Cache hit ratio statistics")
            plt.xlabel("Time")
            plt.ylabel("cache rate")
            plt.show()

    def test_content_classify(self):
        env = Enviroment.System(50, 500)
        env.run()
        print(Enviroment.System.content_by_theme)
        print(Enviroment.System.content_by_form)
...
def transfer_vehicle_no(vehicle_list):
    ans = []
    for vehicle in vehicle_list:
        ans.append(vehicle.vehicle_no)
    return ans